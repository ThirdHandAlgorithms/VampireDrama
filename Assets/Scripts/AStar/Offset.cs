...
namespace RoyT.AStar
{
    ...
    public struct Offset : IEquatable<Offset>
    {
        ...
        {
            if (x < -1 || x > 1)
                throw new ArgumentOutOfRangeException("Parameter x cannot have a magnitude larger than one");

            if (y < -1 || y > 1)
                throw new ArgumentOutOfRangeException("Parameter y cannot have a magnitude larger than one");

            if (x == 0 && y == 0)
                throw new ArgumentException("Parameters x and y cannot both be zero");
        ...
        {
            return "Offset: (" + this.X.ToString() + ", " + this.Y.ToString() + ")";
        }
