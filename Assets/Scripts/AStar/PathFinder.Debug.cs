...
namespace RoyT.AStar
{
    ...
    {
        internal static List<Step> StepList { get; set; }

        private static void EnsureStepList()
        {
            if (StepList == null) StepList = new List<Step>(0);
        }

        [Conditional("DEBUG")]
        private static void MessageCurrent(Position position, IList<Position> path)
        {
            EnsureStepList();
    ...
    internal class Step
    {
        public Step(StepType type, Position position, IList<Position> path)
        {
            this.Type = type;
